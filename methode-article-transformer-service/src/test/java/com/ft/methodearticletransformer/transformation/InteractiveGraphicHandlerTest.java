...
public class InteractiveGraphicHandlerTest extends BaseXMLEventHandlerTest {
    ...
    private InteractiveGraphicHandler eventHandler;
    ...
    public void setup() throws Exception  {
        eventHandler = new InteractiveGraphicHandler(fallbackHandler);
    }
    ...
    private void shouldTransform(final String url) throws Exception {
    ...
    public void shouldNotTransformBlacklist() throws Exception {
        for (final String notInteractiveGraphicUrl : BLACKLIST)
            shouldNotTransform(notInteractiveGraphicUrl);
    }

    @SuppressWarnings("unchecked")
    private void shouldNotTransform(final String url) throws Exception {
        final Map<String, String> attributes = new HashMap<>();
        attributes.put(SRC, url);
        StartElement startElement = getStartElementWithAttributes(IFRAME, attributes);
        final BodyWriter mockBodyWriter = mock(BodyWriter.class);
    ...
    public void shouldTransformWidthAndHeight() throws Exception {
        final String url = WHITELIST.get(0);
        final Map<String, String> attributes = new HashMap<>();
        attributes.put(SRC, url);
