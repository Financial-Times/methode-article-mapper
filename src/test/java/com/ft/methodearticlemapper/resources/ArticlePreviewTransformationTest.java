...
public class ArticlePreviewTransformationTest {
  ...
  private static final String VALUE_PROPERTY =
      FileUtils.readFile("preview/article_preview_value.xml");
  private static final String ATTRIBUTES_PROPERTY =
      FileUtils.readFile("preview/article_preview_attributes.xml");
  private static final String SYSTEM_ATTRIBUTES_PROPERTY =
      FileUtils.readFile("preview/article_preview_system_attributes.xml");
  ...
  /**
   * Classes under test - validation for successful transformation of an article preview occurs in
   * both classes
   */
  ...
  public void setUp() {
    ...
    postContentToTransformResource =
  ...
  /**
   * Tests that an unpublished article preview contains minimal required data to transform it to UP
   * content.
   */
  @Test
  ...
  /**
   * Tests that validation fails for an article preview when the EomFile type is not an
   * EOM::CompoundStory.
   */
