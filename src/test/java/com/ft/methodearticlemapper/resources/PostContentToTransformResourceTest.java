...
public class PostContentToTransformResourceTest {
  ...
  public void thatIfMapPreviewParamIsTruePreviewProcessingIsTriggered() {
    PostContentToTransformResource resource =
  ...
  public void thatModeQueryParameterIsPassedThrough() {
    ...
    verify(eomFileProcessor)
        .process(eq(eomFile), eq(TransformationMode.SUGGEST), eq(TRANSACTION_ID), any());
  }
  ...
  public void thatLastModifiedComesFromNativeWhenSourceIsNative() {
    PostContentToTransformResource resource =
        new PostContentToTransformResource(
            eomFileProcessor,
            PropertySource.fromNative,
            PropertySource.fromTransaction,
            "publishReference");
    String lastModified = "2017-12-01T14:32:27Z";
    Date lastModifiedDate =
        Date.from(Instant.from(DateTimeFormatter.ISO_OFFSET_DATE_TIME.parse(lastModified)));

    when(eomFile.getAdditionalProperties())
        .thenReturn(Collections.singletonMap("lastModified", lastModified));
    resource.map(eomFile, true, TransformationMode.PUBLISH.toString().toLowerCase());

    verify(eomFileProcessor)
        .process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), eq(lastModifiedDate));
  }
  ...
  public void thatLastModifiedNullIsAllowedWhenSourceIsNative() {
    ...
    verify(eomFileProcessor)
        .process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), isNull(Date.class));
  }
  ...
  public void thatLastModifiedIsNowWhenSourceIsTransaction() {
    ...
    verify(eomFileProcessor)
        .process(
            ...
            eq(TRANSACTION_ID),
            argThat(allOf(greaterThanOrEqualTo(before), lessThanOrEqualTo(after))));
  }
  ...
  public void thatTransactionIdComesFromNativeWhenSourceIsNative() {
    ...
    when(eomFile.getAdditionalProperties())
        .thenReturn(Collections.singletonMap("publishReference", txId));
    resource.map(eomFile, true, TransformationMode.PUBLISH.toString().toLowerCase());
