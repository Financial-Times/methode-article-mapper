...
public class PostContentToTransformResourceForPublicationUnhappyPathsTest {
  ...
  /*Class under test*/
  private PostContentToTransformResource postContentToTransformResource = new PostContentToTransformResource(
      eomFileProcessor, PropertySource.fromTransaction, PropertySource.fromTransaction, "publishReference");
  ...
  public void shouldThrow422ExceptionWhenWorkflowStatusNotEligibleForPublishing() {
    ...
    try {
      ...
      assertThat(((ErrorEntity) wace.getResponse().getEntity()).getMessage(), equalTo(
          String.format("Workflow status [%s] not eligible for publishing", workflowStatusNotEligibleForPublishing)));
      assertThat(wace.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
  ...
  public void shouldThrow422ExceptionWhenMethodeFieldMissing() {
    ...
    try {
      ...
      assertThat(((ErrorEntity) wace.getResponse().getEntity()).getMessage(), equalTo(
          String.format(PostContentToTransformResource.ErrorMessage.METHODE_FIELD_MISSING.toString(), missingField)));
      assertThat(wace.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
  ...
  public void shouldThrow422ExceptionWhenMethodeBodyMissing() {
    ...
    try {
      ...
      assertThat(((ErrorEntity) e.getResponse().getEntity()).getMessage(), containsString(uuid.toString()));
  ...
  public void shouldThrow422ExceptionWhenMethodeBodyBlankAfterTransformation() {
    ...
    try {
      ...
      assertThat(((ErrorEntity) e.getResponse().getEntity()).getMessage(), containsString("it's blank"));
      assertThat(e.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
  ...
  public void shouldThrow422ExceptionWhenXMLCannotBeParsed() {
    ...
    try {
      ...
      assertThat(((ErrorEntity) e.getResponse().getEntity()).getMessage(), containsString("dodgy xml"));
      assertThat(e.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
