 ...
 *
 * <p>Created by julia.fernee on 29/01/2016.
 */
public class PostContentToTransformResourceForPublicationUnhappyPathsTest {
  ...
  /**
   * Test that the response contains http error code 400 and the correct message when uuid path
   * parameter is absent.
   */
  @Test
  public void shouldThrow400ExceptionWhenNoUuidPassed() {
    try {
      ...
      postContentToTransformResource.map(eomFile, IS_PREVIEW_FALSE, null);
  ...
  /**
   * Tests that response contains 400 error code and the correct message when uuid path variable
   * does not conform to UUID format.
   */
  @Test
  public void shouldThrow400ExceptionWhenInvalidUuidPassed() {
    try {
      EomFile eomFile =
          ArticlePreviewTransformationTest.articlePreviewMinimalEomFile("invalid_uuid");
      postContentToTransformResource.map(eomFile, IS_PREVIEW_FALSE, null);
      fail("No exception was thrown, but expected one.");
    } catch (WebApplicationClientException wace) {
      assertThat(
  ...
  /**
   * Tests that response contains 404 error code and the correct message when content marked as
   * deleted in Methode is attempted to be published.
   */
  @Test
  public void shouldThrow404ExceptionWhenContentIsMarkedAsDeletedInMethode() {
    when(eomFileProcessor.process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), any()))
        .thenThrow(new MethodeMarkedDeletedException(uuid, "FT"));
    try {
  ...
  /**
   * Tests that the response contains http code 422 and the correct message when the type property
   * in the json payload is not EOM::CompoundStory.
   */
  @Test
  ...
  /**
   * Tests that response contains 422 error code and the correct message when content that marked
   * with an active publication embargo date is attempted to be published.
   */
  @Test
  ...
  /**
   * Tests that response contains 422 error code and the correct message when web channel element in
   * eom file system attributes property indicates that the content is not eligible for publication.
   */
  @Test
  ...
  /**
   * Tests that response contains 422 error code and the correct message when web source element in
   * eom file attributes property indicates that the content is not eligible for publication.
   */
  @Test
  ...
  /**
   * Tests that response contains 422 error code and the correct message when content with workFlow
   * status ineligible for publication is attempted to be published.
   */
  @Test
  public void shouldThrow422ExceptionWhenWorkflowStatusNotEligibleForPublishing() {
    ...
    when(eomFileProcessor.process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), any()))
        .thenThrow(
            new WorkflowStatusNotEligibleForPublishException(
                uuid, workflowStatusNotEligibleForPublishing));
    try {
      postContentToTransformResource.map(eomFile, IS_PREVIEW_FALSE, null);
      fail("No exception was thrown, but expected one.");
    } catch (WebApplicationClientException wace) {
      assertThat(
          ...
          equalTo(
              String.format(
                  "Workflow status [%s] not eligible for publishing",
  ...
  /**
   * Tests that response contains 422 error code and the correct message when content with missing
   * publish date is attempted to be published.
   */
  @Test
  public void shouldThrow422ExceptionWhenMethodeFieldMissing() {
    ...
    try {
      ...
      assertThat(
          ...
          equalTo(
              String.format(
                  PostContentToTransformResource.ErrorMessage.METHODE_FIELD_MISSING.toString(),
                  missingField)));
      assertThat(wace.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
  ...
  /**
   * Tests that response contains 422 error code and the correct message when eom-file with missing
   * or empty value property or its value property translates into an empty content body is
  ...
  public void shouldThrow422ExceptionWhenMethodeBodyMissing() {
    ...
    try {
      ...
      assertThat(
          ((ErrorEntity) e.getResponse().getEntity()).getMessage(),
  ...
  public void shouldThrow422ExceptionWhenMethodeBodyBlankAfterTransformation() {
    ...
    try {
      ...
      assertThat(
          ((ErrorEntity) e.getResponse().getEntity()).getMessage(), containsString("it's blank"));
      assertThat(e.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }

  @Test
  public void shouldThrow422ExceptionWhenXMLCannotBeParsed() {
    when(eomFileProcessor.process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), any()))
        .thenThrow(new TransformationException(new SAXException("dodgy xml")));
    try {
      postContentToTransformResource.map(eomFile, IS_PREVIEW_FALSE, null);
      fail("No exception was thrown, but expected one.");
    } catch (WebApplicationClientException e) {
      assertThat(
          ((ErrorEntity) e.getResponse().getEntity()).getMessage(), containsString("dodgy xml"));
      assertThat(e.getResponse().getStatus(), equalTo(HttpStatus.SC_UNPROCESSABLE_ENTITY));
    }
  }
  ...
  public void thatUnsupportedTransformationModeIsRejected() {
    when(eomFileProcessor.process(
            eq(eomFile), eq(TransformationMode.PUBLISH), eq(TRANSACTION_ID), any()))
        .thenThrow(
            new UnsupportedTransformationModeException(
                uuid.toString(), TransformationMode.PUBLISH));

    try {
      postContentToTransformResource.map(eomFile, false, TransformationMode.PUBLISH.toString());
      fail("No exception was thrown, but expected one.");
    } catch (WebApplicationClientException e) {
      assertThat(
          ((ErrorEntity) e.getResponse().getEntity()).getMessage(),
