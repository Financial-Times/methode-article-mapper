...
public class PromoBoxEventHandlerTest extends BaseXMLEventHandlerTest {
  ...
  private static final String LINK_VALUE =
      "<a href=\"http://www.ft.com/cms/s/0/0bdf4bb6-6676-11e4-8bf6-00144feabdc0.html\"/>";
  ...
  public void shouldThrowBodyProcessingExceptionIfOpeningTagIsNotPromoBox() throws Exception {
    StartElement startElement =
        getStartElementWithAttributes(INCORRECT_ELEMENT, attributeClassEqualToNumberComponent());
  ...
  public void shouldNotTransformContentIfBigNumberAndAllValidDataIsNotPresent() throws Exception {
    StartElement startElement =
        getStartElementWithAttributes(
            METHODE_PROMO_BOX_ELEMENT, attributeClassEqualToNumberComponent());
    when(mockPromoBoxXMLParser.parseElementData(
            startElement, mockXmlEventReader, mockBodyProcessingContext))
        .thenReturn(mockPromoBoxData);
  ...
  @Test
  public void shouldClosePtagsAndWriteTransformedBigNumberElementsToWriterOutsideOfPtags()
      throws Exception {
  ...
  @Test
  public void shouldWriteTransformedPromoBoxElementsToWriterWithTwoElementsOutsideClosedPtags()
      throws Exception {
    when(mockBodyWriter.isPTagCurrentlyOpen()).thenReturn(false);
    shouldWriteTransformedPromoBoxElementsToWriter();
    verify(mockBodyWriter, never()).writeEndTag(PARAGRAPH_TAG);
  ...
  @Test
  public void
      shouldClosePtagsAndWriteTransformedPromoBoxElementsToWriterWithTwoElementsOutsideOfPtags()
          throws Exception {
    when(mockBodyWriter.isPTagCurrentlyOpen()).thenReturn(true);
    shouldWriteTransformedPromoBoxElementsToWriter();
    verify(mockBodyWriter).writeEndTag(PARAGRAPH_TAG);
  ...
  @Test
  public void shouldWriteTransformedPromoBoxElementsToWriterWithFourElementsOutsideClosedPtags()
      throws Exception {
    when(mockBodyWriter.isPTagCurrentlyOpen()).thenReturn(false);
    when(mockPromoBoxData.getTitle()).thenReturn(TITLE_VALUE);
    when(mockPromoBoxData.getLink()).thenReturn(LINK_VALUE);
  ...
  @Test
  public void
      shouldClosePtagsAndWriteTransformedPromoBoxElementsToWriterWithFourElementsOutsideOfPtags()
          throws Exception {
    when(mockBodyWriter.isPTagCurrentlyOpen()).thenReturn(true);
    when(mockPromoBoxData.getTitle()).thenReturn(TITLE_VALUE);
    when(mockPromoBoxData.getLink()).thenReturn(LINK_VALUE);
  ...
  public void shouldWriteTransformedPromoBoxElementsToWriter() throws Exception {
    StartElement startElement =
        getStartElementWithAttributes(METHODE_PROMO_BOX_ELEMENT, new HashMap<String, String>());
    when(mockPromoBoxXMLParser.parseElementData(
            startElement, mockXmlEventReader, mockBodyProcessingContext))
        .thenReturn(mockPromoBoxData);
  ...
  public void shouldPassThroughClassAttributeOnPromoBox() throws Exception {
    ...
    verify(mockBodyWriter)
        .writeStartTag(PROMO_BOX_ELEMENT, singletonMap(CLASS_ARTICLE, RELATED_ARTICLE_CLASS));
  }
  ...
  private Map<String, String> attributeClassEqualToNumberComponent() {
    ...
    attributeClassEqualToNumberComponent.put(
        PromoBoxEventHandler.PROMO_CLASS_ATTRIBUTE, PromoBoxEventHandler.NUMBERS_COMPONENT_CLASS);
