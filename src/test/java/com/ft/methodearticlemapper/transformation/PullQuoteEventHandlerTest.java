...
public class PullQuoteEventHandlerTest extends BaseXMLEventHandlerTest {
  ...
  public void shouldNotTransformContentIfAllValidDataIsNotPresent() throws Exception {
    ...
    when(mockPullQuoteXMLParser.parseElementData(
            startElement, mockXmlEventReader, mockBodyProcessingContext))
        .thenReturn(mockPullQuoteData);
  ...
  @Test
  public void shouldNotWriteTransformedContentIfAllValidDataIsNotPresentAfterTransformation()
      throws Exception {
    StartElement startElement = getStartElementWithAttributes(PULL_QUOTE_ELEMENT, noAttributes());
    when(mockPullQuoteXMLParser.parseElementData(
            startElement, mockXmlEventReader, mockBodyProcessingContext))
