...
public class BodyProcessingFieldTransformerFactory implements FieldTransformerFactory {
  private static final EnumSet<TransformationMode> NOT_SUGGEST =
      EnumSet.complementOf(EnumSet.of(TransformationMode.SUGGEST));
  ...
  public BodyProcessingFieldTransformerFactory(
    ...
    xpathHandlers =
        ImmutableMap.of(
            "//company", new TearSheetLinksTransformer(concordanceApiClient, concordanceApiUri));
  ...
  private BodyProcessor stAXTransformingBodyProcessor() {
    return new StAXTransformingBodyProcessor(
        new MethodeBodyTransformationXMLEventHandlerRegistry(
            videoMatcher, interactiveGraphicsMatcher, canonicalUrlTemplate));
  }

  private BodyProcessor methodeLinksBodyProcessor() {
    return new ModalBodyProcessor(
        new MethodeLinksBodyProcessor(
            documentStoreApiClient, documentStoreUri, canonicalUrlTemplate),
        NOT_SUGGEST);
  }
  ...
  private BodyProcessor stripByAttributesAndValuesBodyProcessor() {
    return new StAXTransformingBodyProcessor(
        new StripByPredefinedAttributesAndValuesEventHandlerRegistry());
  }
}
