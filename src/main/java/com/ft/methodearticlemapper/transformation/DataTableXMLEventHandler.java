...
public class DataTableXMLEventHandler extends BaseXMLEventHandler {
  ...
  private static final String DATA_TABLE_LAYOUT_SMALLSCREEN_ATTRIBUTE =
      "data-table-layout-smallscreen";
  private static final String DATA_TABLE_LAYOUT_LARGESCREEN_ATTRIBUTE =
      "data-table-layout-largescreen";
  ...
  public DataTableXMLEventHandler(
      XmlParser<DataTableData> dataTableDataXmlParser,
    ...
    checkNotNull(
        stripElementAndContentsXMLEventHandler,
        "stripElementAndContentsXMLEventHandler cannot be null");
  ...
  public void handleStartElementEvent(
    ...
    if (isElementOfCorrectType(startElement)) {
      ...
      DataTableData dataBean =
          dataTableDataXmlParser.parseElementData(
      ...
      stripElementAndContentsXMLEventHandler.handleStartElementEvent(
          startElement, xmlEventReader, eventWriter, bodyProcessingContext);
    }
  }

  private void writeDataTable(
      BodyWriter eventWriter, DataTableData dataBean, StartElement startElement) {
  ...
  protected boolean isElementOfCorrectType(StartElement event) {
    if (event
        ...
        .getLocalPart()
        .toLowerCase()
        .equals(DATA_TABLE_HTML_ELEMENT_NAME.toLowerCase())) {
      Attribute classAttr = event.getAttributeByName(QName.valueOf(DATA_TABLE_ATTRIBUTE_NAME));
      if (classAttr != null
          && classAttr.getValue().toLowerCase().equals(DATA_TABLE_ATTRIBUTE_VALUE)) {
        return true;
      }
    }
    return false;
