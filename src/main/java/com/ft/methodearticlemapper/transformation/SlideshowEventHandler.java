...
public class SlideshowEventHandler extends BaseXMLEventHandler {
  ...
  protected SlideshowEventHandler(
      XmlParser<SlideshowData> slideshowXMLParser, String slideshowUrlTemplate) {
  ...
  private void transformFieldContentToStructuredFormat(
  ...
  private SlideshowData parseElementData(
  ...
  private Map<String, String> getValidAttributes(SlideshowData dataBean) {
    ...
    validAttributes.put(
        DATA_EMBEDDED,
        YEP); // If we know it's a slideshow, it's embedded. Otherwise it's just a link.
