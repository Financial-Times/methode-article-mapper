...
public class DynamicContentXMLEventHandler extends BaseXMLEventHandler {
  ...
  private static final String DYNAMIC_CONTENT_TYPE =
      "http://www.ft.com/ontology/content/DynamicContent";
  ...
  public void handleStartElementEvent(
    ...
    DynamicContentData dynamicContentData =
        dynamicContentXMLParser.parseElementData(event, xmlEventReader, bodyProcessingContext);
    ...
    attributes.put(
        "url", String.format("http://%s/content/%s", apiHost, dynamicContentData.getUuid()));
  ...
  private String getApiHostFromContext(final BodyProcessingContext bodyProcessingContext) {
    ...
    final MappedDataBodyProcessingContext mappedDataBodyProcessingContext =
        (MappedDataBodyProcessingContext) bodyProcessingContext;
