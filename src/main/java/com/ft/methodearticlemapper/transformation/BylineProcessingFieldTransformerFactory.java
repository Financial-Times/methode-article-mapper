...
public class BylineProcessingFieldTransformerFactory implements FieldTransformerFactory {
  ...
  private List<BodyProcessor> bodyProcessors() {
    return asList(
        ...
        new RemoveXMLEntityEscapingBodyProcessor(), // convert from XML back to plain text by
        // unencoding escaped characters
  ...
  private BodyProcessor stAXTransformingBodyProcessor() {
    return new StAXTransformingBodyProcessor(
        new MethodeBylineTransformationXMLEventHandlerRegistry());
  }
