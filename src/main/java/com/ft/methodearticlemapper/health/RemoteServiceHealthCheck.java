...
public class RemoteServiceHealthCheck extends AdvancedHealthCheck {
  ...
  public RemoteServiceHealthCheck(
    ...
    this.remoteServiceUri =
        UriBuilder.fromPath(path).scheme("http").host(hostName).port(port).build();
  }
  ...
  public AdvancedResult checkAdvanced() throws Exception {
    ...
    try {
      response =
          client.resource(remoteServiceUri).header("Host", hostHeader).get(ClientResponse.class);
      ...
      String message =
          getName()
              + ": "
              + "Exception during remote service check call, "
  ...
  protected String technicalSummary() {
    return String.format(
        "This service is not able to connect to %s at %s", serviceName, remoteServiceUri);
  }
