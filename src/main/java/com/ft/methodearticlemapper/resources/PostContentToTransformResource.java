...
public class PostContentToTransformResource {
  ...
  public final Content map(
      EomFile eomFile, @QueryParam("preview") boolean preview, @QueryParam("mode") String mode) {
    ...
    if (mode == null) {
      ...
      try {
        ...
        throw ClientError.status(SC_BAD_REQUEST).context(uuid).error(e.getMessage()).exception(e);
      }
    }
      ...
      case fromNative:
        ...
        if (!Strings.isNullOrEmpty(s)) {
          try {
            ...
            LOG.warn(
                "Invalid value for lastModified: uuid={} lastModified={}", eomFile.getUuid(), s);
          }
        }
        break;
  ...
  private Content processRequest(
      String uuid,
    ...
    try {
      ...
      throw ClientError.status(SC_UNPROCESSABLE_ENTITY).error(e.getMessage()).exception(e);
      ...
      throw ClientError.status(SC_NOT_FOUND).context(uuid).error(e.getMessage()).exception(e);
    }
  }
  ...
  private void validateUuid(String uuid) {
    if (uuid == null) {
      throw ClientError.status(SC_BAD_REQUEST)
          .context(null)
          .reason(ErrorMessage.UUID_REQUIRED)
          .exception();
    }
    try {
      ...
      throw ClientError.status(SC_BAD_REQUEST).reason(ErrorMessage.INVALID_UUID).exception(iae);
    }
  }
